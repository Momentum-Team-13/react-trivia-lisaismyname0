...
export default function OneQuestion ({index, questions, correctAnswerBank, setIndex, shuffleButtons})
{
    ...
    <div className = 'result'>{correct ? ("You chose the correct answer!"):("")}</div>


    {incorrect ? (<div className="result">
Sorry the correct answer was: {currentQuestion.correct_answer}
    ...
    <div>
    <Count rightCount ={rightCount} wrongCount={wrongCount} correctAnswerBank={correctAnswerBank}/></div>
    </div>    
)
}

const QuestionCount = ({questionCount, correctAnswerBank, setNextButton, nextButton, handleNext, userAnswerBank, settleScore}) => {
...
const FinalPage = ({userAnswerBank, correctAnswerBank,}) => {
    return(
        <div className= 'finalAnswers'>
            <div>
                <strong><p>Your Answers</p></strong> 
                {userAnswerBank.map((answer)=>(<li key={answer}>{answer}</li>))}
            </div>
            <div>
                <strong><p>Correct Answers</p></strong> {correctAnswerBank.map((answer)=>(<li key={answer}>{answer}</li>))}
            </div>
        </div>
    )
}
